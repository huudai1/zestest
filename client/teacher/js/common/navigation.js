...
const Navigation = {
    ...
    async showStep(stepNumber) {
        ...
        if (stepNumber > 1 && !StorageManager.examData.name) {
            stepNumber = 1; 
        }
                ...
                if (typeof window.finishTest === 'function') {
                    ...
                    if (success) {
                        ...
                        if (typeof window.showPage === 'function') {
                            ...
                            document.getElementById('page-create-steps').style.display = 'none';
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 2. Chạy điều hướng trang
    await Navigation.showStep(initialStep);

    // 3. Khôi phục Modal nếu có
    Navigation.restoreModalDraft();
