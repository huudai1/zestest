const ZestUnpackager = {
    ...
    async unpack(zipData) {
        ...
        // 1. Giải nén manifest.json (Meta dữ liệu)
        ...
        if (manifestFile) {
            ...
            throw new Error("Không tìm thấy file manifest.json trong gói đề!");
        }
        ...
        if (audioFile) {
            ...
            // Tạo URL để chơi nhạc mà không tốn RAM load chuỗi base64
        ...
        result.images = await Promise.all(imageFiles.map(async (file) => {
            ...
            return {
                name: file.name,
                url: URL.createObjectURL(imgBlob) // Tạo URL để gán vào thẻ <img>
            };
        }));
