...
const PdfProcessor = {
    ...
    async process(file) {
        ...
        try {
            ...
            // --- KIỂM TRA HẠN MỨC CHO GUEST ---
            const userId = (typeof ClientInternal !== 'undefined') ? ClientInternal.getExistingId() : "";
            if (userId.startsWith('gst_') && numPages > 70) {
                const err = new Error(`Hạn mức Guest là 20 trang. File của bạn có ${numPages} trang.`);
                err.isLimitError = true;
                err.feature = "Số trang PDF";
                err.limit = "20";
                err.messageDetail = "Hãy đăng ký để nâng tổng số trang lên 60!";
                throw err;
            }
