const QuizController = {
    ...
    save(qNum, val) {
        ...
        // GỬI LÊN WEBSOCKET CHO GIÁO VIÊN
        if (window.examWs && window.examWs.readyState === WebSocket.OPEN) {
            const stu_answer = StorageManager.examData.stu_answer;
            // Bỏ qua phần tử index 0 (vì mảng 1-indexed) và đếm những câu đã trả lời
            const done = stu_answer.filter((a, idx) => idx > 0 && a && a.trim() !== "").length;
